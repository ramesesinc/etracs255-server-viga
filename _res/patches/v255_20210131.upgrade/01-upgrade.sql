...
;
alter table account_item_mapping modify itemid varchar(50) not null 
;
create index ix_itemid on account_item_mapping (itemid) 
;
...
update 
  ...
  ( 
    select 
      a.objid, 'SRE-V254' as maingroupid, a.parentid as groupid 
    from sreaccount a, sreaccount p  
    where a.parentid is not null 
      and a.parentid = p.objid 
      and a.type = 'group' 
...
from (  
  select 
    concat('', convert(a.year,char) ,'|', a.objid) as objid, 
...
from ( 
  select 
    objid, 'NGAS' as maingroupid, code, title, null as groupid, 
    'root' as type, null as leftindex, null as rightindex, null as `level` 
  from ngasaccount where parentid is null
  union all 
  select 
    a.objid, 'NGAS' as maingroupid, a.code, a.title, null as groupid, 
    'group' as type, null as leftindex, null as rightindex, null as `level` 
...
update 
  ...
  ( 
    select 
      a.objid, 'NGAS' as maingroupid, a.parentid as groupid 
    from ngasaccount a, ngasaccount p  
    where a.parentid is not null 
      and a.parentid = p.objid 
      and a.type = 'group' 
...
select 
  r.businessid, r.account_objid, count(*) as icount, 
  (
    select objid from business_recurringfee 
    where businessid = r.businessid and account_objid = r.account_objid 
    order by amount desc limit 1 
  ) as objid 
from business_recurringfee r 
group by r.businessid, r.account_objid 
having count(*) > 1
;
create table ztmp_fix_business_recurringfee_for_removal 
select r.objid 
from business_recurringfee r, ztmp_fix_business_recurringfee z 
where r.businessid = z.businessid 
  and r.account_objid = z.account_objid 
  and r.objid <> z.objid 
;
drop table if exists ztmp_fix_business_recurringfee
;
delete from business_recurringfee where objid in (select objid from ztmp_fix_business_recurringfee_for_removal) 
;
drop table if exists ztmp_fix_business_recurringfee_for_removal
;
...
;
update sys_rule set _ukey=MD5(objid) where _ukey=''
;
